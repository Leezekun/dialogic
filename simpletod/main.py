...
def _sorted_checkpoints(args):
    ...
    for dir in dirs:
        if "-" in dir:
            idx = int(dir.split("-")[1])
            idx_dirs.append((idx, dir))
    sorted(idx_dirs, key=lambda idx_dirs: idx_dirs[0])
    sorted_dirs = []
    for idx_dir in idx_dirs:
        sorted_dirs.append(os.path.join(args.output_dir, idx_dir[1]))
    return sorted_dirs
...
def main():
    ...
    if args.do_eval and args.local_rank in [-1, 0]:
        ...
        if args.eval_all_checkpoints:
            ...
            logger.info("Evaluate the last checkpoints: {}".format(checkpoints))

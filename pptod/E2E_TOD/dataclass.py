...
class MultiWozData:
    def __init__(self, model_name, tokenizer, cfg, data_path_prefix, shuffle_mode='shuffle_session_level', 
        data_mode='train', data_version='2.0', use_db_as_input=True, cascaded=False, add_special_decoder_token=True, train_data_ratio=1.0, aug_train_data_file="", aug_train_data_num=-1):
        '''
        ...
        if data_mode in ['train', 'interact']:
            ...
            if aug_train_data_file:
                ...
                aug_train_json_path = os.path.join(data_path_prefix, processed_data_path, aug_train_data_file)
            ...
            if self.train_data_ratio < 1.0:
                ...
                # random.shuffle(train_raw_data) # no shuffle, so that we can use the same set of training data everytime
            ...
            if aug_train_data_file:
                if aug_train_data_num > 0:
                    random.shuffle(aug_train_raw_data)
                    aug_train_raw_data = aug_train_raw_data[:aug_train_data_num]

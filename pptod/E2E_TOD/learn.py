...
def parse_config():
    ...
    parser.add_argument('--aug_train_data_num', type=int, default=-1, help='the augmented data file path')
...
if __name__ == '__main__':
    ...
    for epoch in range(args.epoch_num):
        ...
        if args.train_data_ratio <= 0.1:
            ...
            if epoch < 5: # first train 5 epoches
                continue
        else:
            pass
        # **********************************************************************
        ...
        if epoch % evaluation_epoch == 0:
            ...
            with torch.no_grad():
                ...
                if cascaded:
                    one_dev_str = 'cascaded_test_e2e_evaluation_inform_{}_success_{}_bleu_{}_combine_score_{}'.format(round(dev_match, 2),
                    round(dev_success,2), round(dev_bleu,2), round(dev_score,2))
                else:
                    one_dev_str = 'test_e2e_evaluation_inform_{}_success_{}_bleu_{}_combine_score_{}'.format(round(dev_match, 2),
                    round(dev_success,2), round(dev_bleu,2), round(dev_score,2))
                ...
                if dev_score > max_dev_score:
                    ...
                    for fname in os.listdir(test_output_dir):
                        if fname.startswith('epoch') and "best" not in fname: # don't remove the best ckpt, epoch_x_best_ckpt

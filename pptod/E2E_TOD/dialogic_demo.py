...
if __name__ == '__main__':
    ...
    with torch.no_grad():
        ...
        if input_user_goal:
            ...
            print()
            print(Fore.CYAN + "Available domains and slot:" + Style.RESET_ALL)
            print(informable_slots)
            print()

            print(Fore.LIGHTGREEN_EX)
            print("The format is ([domain1] slot_name1 slot_value1 slot_name2 slot_value2 ... [domain2] slot_name1 slot_value1 slot_name2 slot_value2 ...), such as ([restaurant] food american people 2 [hotel] day monday stay 1).")
            print(Style.RESET_ALL)

            has_valid_goal = False
            while not has_valid_goal:
                print()
                augment_goal = input(Fore.LIGHTMAGENTA_EX + "Input your user goal or 'r' to randomly generate one:" + Style.RESET_ALL)
                if augment_goal in ['r', 'random']:
                    input_user_goal = False
                    break
                try:
        ...
        for dial_id, dialog_turn_with_info in augment_dialogs.items():
            ...
            print(Fore.RED)
            print(f"Start simulation based on {dial_id}!")
            print(f"Original user goal: {paser_dict_to_bs(augment_goal)}.")
            print(Style.RESET_ALL)

            while turn_id < args.max_turn_num:

                if pause:
                    _ = input(Fore.BLACK + f"Press ENTER to continue simulating turn {turn_id}, or q to quit:" + Style.RESET_ALL)
                ...
                if debug: print(Fore.LIGHTMAGENTA_EX + f"Original GPT-3 generation of User: " + Style.RESET_ALL + f"{user_with_bs}" )
                ...
                if verify_bs or verify_da:
                    ...
                    if debug: print(Fore.LIGHTBLUE_EX + f"Verifier predicted belief state: " + Style.RESET_ALL + f"{one_bs_text}" + Style.RESET_ALL)
                ...
                if debug: print(Fore.LIGHTRED_EX + f"Corrected belief state: " + Style.RESET_ALL + f"{bs_text}")
                # correct belief state
                one_bs_text = bs_text
                # record turn info
                ...
                if debug: print(Fore.LIGHTMAGENTA_EX + f"Corrected GPT-3 generation of User: " + Style.RESET_ALL +f"{user_text}")
                prompt += "\n" + user_text
                logger.info("\n" + user_text)
                ...
                if input_contain_db:
                    # record turn info
                    if debug: print(Fore.LIGHTBLUE_EX + f"DB search result: " + Style.RESET_ALL + f"{one_queried_db_result}")
                ...
                if verify_da:
                    ...
                    if debug: print(Fore.LIGHTYELLOW_EX + f"Verifier generated Action: " + Style.RESET_ALL + f"{one_da_text}")
                    ...
                    if one_da_text:
                        ...
                        if debug: print(Fore.CYAN + f"Verifier generated Response: " + Style.RESET_ALL + f"{one_nlg_text}")
                        ...
                        logger.info("\n" + system_prefix(one_da_text, system_based_on_da))
                        if debug: print(Fore.LIGHTMAGENTA_EX + f"Corrected GPT-3 generation of System: " + Style.RESET_ALL + f"{system_prefix(one_da_text, system_based_on_da)}")
                ...
                if not one_da_text:
                    ...
                    if "Assistant(" in system_with_da:
                        ...
                        if debug: print(Fore.LIGHTGREEN_EX + f"GPT-3 predicted dialog action: {gpt3_aspn_reform}" + Style.RESET_ALL)
                    else:
                        gpt3_aspn_reform = ""
                ...
                print(Fore.GREEN + "User: " + Style.RESET_ALL + f"{turn_info['user']}" )
                print(Fore.GREEN + "System: " + Style.RESET_ALL + f"{turn_info['resp']}" )
                print("--------------------------------------------------------------------------")
                print()
            ...
            print(f"Final user goal: {paser_dict_to_bs(real_goal)}.")
            print()

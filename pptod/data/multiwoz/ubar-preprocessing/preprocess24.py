...
from config24 import global_config as cfg
...
def get_db_values(value_set_path): # value_set.json, all the domain[slot] values in datasets
    ...
    with open('data/multi-woz-2.4-processed/bspn_word_collection.json', 'w') as f:
...
class DataPreprocessor(object):
    def __init__(self):
        ...
        data_path = 'data/MultiWOZ_2.4/data.json'
        ...
        self.delex_sg_valdict_path = 'data/multi-woz-2.4-processed/delex_single_valdict.json'
        self.delex_mt_valdict_path = 'data/multi-woz-2.4-processed/delex_multi_valdict.json'
        self.ambiguous_val_path = 'data/multi-woz-2.4-processed/ambiguous_values.json'
        self.delex_refs_path = 'data/multi-woz-2.4-processed/reference_no.json'
    ...
    def preprocess_main(self, save_path=None, is_test=False):
        ...
        self.vocab.save_vocab('data/multi-woz-2.4-processed/vocab')
        with open('data/multi-woz-2.4-analysis/dialog_acts.json', 'w') as f:
            json.dump(ordered_sysact_dict, f, indent=2)
        with open('data/multi-woz-2.4-analysis/dialog_act_type.json', 'w') as f:
...
if __name__=='__main__':
    ...
    if not os.path.exists('data/multi-woz-2.4-processed'):
        os.mkdir('data/multi-woz-2.4-processed')
    dh = DataPreprocessor()
    data = dh.preprocess_main()
    

    with open('data/multi-woz-2.4-processed/data_for_damd.json', 'w') as f:

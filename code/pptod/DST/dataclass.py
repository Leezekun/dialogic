...
class DSTMultiWozData:
    def __init__(self, model_name, tokenizer, data_path_prefix, shuffle_mode='shuffle_session_level',
        data_mode='train', delx=False, add_prefix=True, add_special_decoder_token=True, train_data_ratio=1.0, aug_train_data_file=None, augment_data_balance=False):
        '''
            ...
            delx: whether using delx response or nodelx response 
        ...
        '''
        self.delx = delx
        assert self.delx in [True, False]
        ...
        if data_mode == 'train':
            ...
            if aug_train_data_file:
                print ('Loading augmented training data. ')
                aug_train_json_path = os.path.join(data_path_prefix, aug_train_data_file)
                with open(aug_train_json_path) as f:
                    aug_train_raw_data = json.load(f)
            ...
            if aug_train_data_file:
                train_raw_data += aug_train_raw_data
                print ('Number of augmented training sessions is {}'.format(len(aug_train_raw_data)))

            print ('Tokenizing raw train data...')
            train_data_id_list = self.tokenize_raw_data(train_raw_data)
            self.train_data_list, self.augment_train_data_list = self.flatten_data(train_data_id_list)
            
            # balance the ratio of original training data and augment training data
            print (f'Original train data turns: {len(self.train_data_list)}, augment train data turns: {len(self.augment_train_data_list)}')
            difference = len(self.augment_train_data_list) - len(self.train_data_list)
            if difference > 0:
                add_train_data_list = random.choices(self.train_data_list, k=difference)
            else:
                add_train_data_list = []
            if augment_data_balance:
                self.train_data_list = self.train_data_list + self.augment_train_data_list + add_train_data_list
            else:
                self.train_data_list = self.train_data_list + self.augment_train_data_list
        ...
        # dev_json_path = "../../../../data/multiwoz/data/multi-woz-2.3-fine-processed/multiwoz-fine-processed-dev.json"
        with open(dev_json_path) as f:
            dev_raw_data = json.load(f)
        print ('Tokenizing raw dev data...')
        dev_data_id_list = self.tokenize_raw_data(dev_raw_data)
        self.dev_data_list, _ = self.flatten_data(dev_data_id_list)
        # record training data
        ...
        # test_json_path = "../../../../data/multiwoz/data/multi-woz-2.3-fine-processed/multiwoz-fine-processed-test.json"
        with open(test_json_path) as f:
            test_raw_data = json.load(f)
        print ('Tokenizing raw test data...')
        test_data_id_list = self.tokenize_raw_data(test_raw_data)
        self.test_data_list, _ = self.flatten_data(test_data_id_list)
        # record test data
        ...
        for idx in range(data_num):
            ...
            for turn in raw_data_list[idx]:
                ...
                for key in turn:
                    ...
                    elif key in ['augment_turns']:
                        augment_turns = []
                        for augment_turn in turn[key]:
                            tokenized_augment_turn_dict = {}
                            for k in augment_turn:
                                value_text = augment_turn[k]
                                ...
                                tokenized_augment_turn_dict[k] = value_id
                            augment_turns.append(tokenized_augment_turn_dict)
                        one_turn_dict[key] = augment_turns
                    else:
    ...
    def convert_to_raw_data(self):
        ...
        for idx, dial_id in enumerate(self.train_dial_id_list):
            ...
            one_sess_list = []
            # the first loop, only for original data
            for turn in self.train_id2session_dict[dial_id]:
                ...
                # original data
                if "augment_turn_num" not in turn:
                    one_sess_list.append(one_turn_dict)
            train_dials[dial_id] = one_sess_list

            # the second loop, for augment data
            for turn in self.train_id2session_dict[dial_id]:
                one_turn_dict = {}
                for key in turn:
                    if key in ['dial_id', 'pointer', 'turn_domain', 'turn_num']:
                        ...
                        one_turn_dict[key] = value_text
                # original data
                if "augment_turn_num" in turn:
                    turn_num = int(turn['turn_num'])
                    augment_turn_num = int(turn['augment_turn_num'])
                    augment_one_sess_list = one_sess_list[:turn_num]
                    augment_one_sess_list.append(one_turn_dict)
                    augment_dial_id = f"{dial_id}-{turn_num}-{augment_turn_num}"
                    train_dials[augment_dial_id] = augment_one_sess_list
        p.finish()
    ...
    def flatten_data(self, data):
            ...
            for turn_id in range(turn_num):
                ...
                if self.delx:
                    ...
                    if 'nodelx_resp' not in curr_turn:
                        curr_turn['nodelx_resp'] = ""
                else:
                    if 'nodelx_resp' not in curr_turn:
                        curr_sys_resp = curr_turn['resp']
                        curr_turn['nodelx_resp'] = ""
                    else:
                        curr_sys_resp = curr_turn['nodelx_resp']

                # construct belief state data
                bs_input = previous_context + curr_turn['user']
                bs_input = self.bs_prefix_id + [self.sos_context_token_id] + bs_input[-900:] + [self.eos_context_token_id]
                bs_output = curr_turn['bspn']
                ...
                # if there are augment turns (for dst)
                if 'augment_turns' in curr_turn:
                    for idx, augment_turn in enumerate(curr_turn['augment_turns']):
                        bs_input = previous_context + augment_turn['user']
                        bs_input = self.bs_prefix_id + [self.sos_context_token_id] + bs_input[-900:] + [self.eos_context_token_id]
                        bs_output = augment_turn['bspn']
         
                        augment_data_list.append({'dial_id': one_dial_id,
                            'turn_num': turn_id,
                            'turn_domain': curr_turn['turn_domain'],
                            'augment_turn_num': idx, # idx for the augment data in this turn
                            'prev_context':previous_context,
                            'user': augment_turn['user'],
                            'usdx': augment_turn['user'],
                            'resp': curr_turn['resp'],
                            'nodelx_resp': curr_turn['nodelx_resp'],
                            'bspn': augment_turn['bspn'],
                            'bspn_reform': augment_turn['bspn_reform'],
                            'bsdx': augment_turn['bsdx'],
                            'bsdx_reform': augment_turn['bsdx_reform'],
                            'bs_input': bs_input,
                            'bs_output': bs_output
                            })
        ...
        return data_list, augment_data_list

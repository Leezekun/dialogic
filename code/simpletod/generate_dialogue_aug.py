...
assert len(os.path.split(model_checkpoint)[-1]) > 0
exp_name = os.path.split(model_checkpoint)[0].split('/')[-2]
checkpoint_index = os.path.split(model_checkpoint)[1].split('-')[-1]

print(os.path.split(model_checkpoint))
print(exp_name)
...
for i, dial_name in enumerate(delex_dict):
    if EVAL_SPLIT == 'train' and i > 1000:
        break
    # d = lex_dict[dial_name]
    d_delex = delex_dict[dial_name]
    print('{} [{}/{}] \r'.format(d_delex['name'], i, num_data), end='')
    sys.stdout.flush()
    beliefs_raw = d_delex['belief_raw']
    user = d_delex['input_raw']
    # system = d_delex['target_raw']
    system_delex = d_delex['target_raw']

    # if 'delex' in model_checkpoisssnt:
    #     target_response = system_delex
    # else:
    #     target_response = system

    # we use delex system response in training and inference
    target_response = system_delex
    ...
    generated = []
    model_context = []
    for turn_id, (usr_turn, _) in enumerate(zip(user, system_delex)):
        ...
        if turn_id == 0:
            ...
            for k in range(turn_id):
                ...
                tmp.append('<|system|> {}'.format(system_delex[k].strip()))
        ...
        if dial_name == 'SNG02319.json' or dial_name == 'sng02319':
...
save_name = '{}_{}_{}'.format(exp_name, checkpoint_index, EVAL_SPLIT)
